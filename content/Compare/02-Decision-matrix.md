...
Это **главная практическая заметка** программы обучения. Она отвечает на вопрос «что применять?» по четырём осям: **размер компании, тип бизнеса, стадия, цель руководителя**. На каждое пересечение — конкретный декомпозированный совет.

> **Как пользоваться.** Следует прочитать 4 секции (по размеру / типу / стадии / цели). На пересечении 4 секций получается свой кластер методологий. В конце — синтезирующий блок «соберём в стек».
...
- [ ] Ясно понятна **проблема**, которую нужно решить («у нас execution gap», а не «давайте внедрим OKR»)
- [ ] Есть **CEO sponsor** или это ответственность руководителя?
