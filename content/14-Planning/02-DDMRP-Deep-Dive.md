...
Главная идея: **не везде нужен буфер, а только в стратегических точках**.
...
```
GREEN (зелёная) — over-buffered    ← ОК
YELLOW (жёлтая) — re-order zone    ← планировать заказ
RED (красная) — danger zone         ← срочно!
```
...
Планирование делается **только** на:
- Зону Yellow (ниже зелёного) — pull-сигнал
- Реальные клиентские заказы

**Не** на forecast горизонта.
