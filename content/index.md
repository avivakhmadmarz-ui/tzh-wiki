---
title: "tzh-wiki"
aliases: ["Education hub", "Education", "Программа обучения"]
type: hub
status: active
domain: education
tags: [index, education]
created: 2026-05-07
updated: 2026-05-18
date created: 2026-05-13
date modified: 2026-05-18
---

# tzh-wiki

> [!info] Личный архив автора
> **tzh-wiki** — личная википедия, созданная автором для собственного использования: структурированный конспект знаний для позиции руководителя высшего звена (C-level — высший управленческий уровень) и топ-менеджмента.
>
> Архив ведётся исключительно в личных, образовательных и некоммерческих целях. Материал собран и обобщён из общедоступных источников (книги, статьи, публичные материалы) с указанием соответствующих отсылок. Права на упоминаемые концепции, фреймворки, методики, бренды и публикации принадлежат их авторам и правообладателям; здесь они приводятся в порядке добросовестного цитирования и обзора.

Единая иерархическая программа знаний по операционному управлению: стратегия и финансы, цепочка поставок и закупки, продукт и маркетинг, данные и системы, лидерство, право и риски.



## Иерархия: пять уровней программы

```
УРОВЕНЬ A — СТРАТЕГИЧЕСКИЙ КОНТУР (зачем компания существует)
   ├── 01 Strategy & Business Model
   ├── 02 Corporate Finance
   └── 03 Management Accounting

УРОВЕНЬ B — ФУНКЦИОНАЛЬНАЯ ГЛУБИНА (как работают деньги, цепочка, продукт)
   ├── 04 Supply Chain Management
   ├── 05 Strategic Sourcing & Procurement
   ├── 06 Foreign Trade (ВЭД)
   └── 07 Category Management

УРОВЕНЬ C — ПРОДУКТ И РЫНОК (что мы продаём и как)
   ├── 08 Product Management & R&D
   ├── 09 E-commerce & Marketplaces
   └── 10 Marketing & Consumer Behavior

УРОВЕНЬ D — ДАННЫЕ, СИСТЕМЫ, OPERATIONS (как мы это делаем)
   ├── 11 Analytics & Business Intelligence
   ├── 12 ERP & Digitalization
   ├── 13 Operations Excellence    ← включает существующий [[13-Operations-Excellence/Lean/index|Lean]]
   ├── 14 Planning Methodologies   ← включает [[14-Planning/SOP/index|S&OP]] + [[14-Planning/Other-methodologies/index|Other]]
   └── 15 Project & Portfolio Management

УРОВЕНЬ E — ЛИДЕРСКИЙ СЛОЙ (как мы ведём людей)
   ├── 16 Leadership & Executive Presence
   ├── 17 Goal Setting           ← включает существующий [[17-Goal-Setting/OKR-KPI/index|OKR / KPI / BSC]]
   ├── 18 HR Management
   ├── 19 Org Design & Change Management
   └── 20 Negotiation (Advanced)

УРОВЕНЬ F — КОНТРОЛЬ И ГОРИЗОНТ (правовое, риски, рост)
   ├── 21 Legal & Compliance
   ├── 22 Risk Management & Business Continuity
   ├── 23 ESG & Sustainability
   ├── 24 Cross-Cultural Management
   ├── 25 Academic Track (MBA / PhD)
   └── 26 Soft Skills & Character

УРОВЕНЬ G — ИНДУСТРИЯ-СПЕЦИФИЧНОЕ
   └── 27 Industry-Specific Deep Dives

КРОСС-МОДУЛЬНЫЕ (применимо ко всем)
   ├── Compare (матрица выбора методов)
   ├── Glossary (словарь терминов)
   ├── [[Forecasting/index|Forecasting]] (прогнозирование спроса)
   └── System (личная AI-система автора)
```

## Карта модулей

### Уровень A — Стратегический контур

| # | Модуль | Что внутри |
|---|--------|-----------|
| 01 | [[01-Strategy/index\|Strategy & Business Model]] | Porter, Blue Ocean, BMC, Wardley Maps, Playing to Win, Scenario Planning, OGSM |
| 02 | [[02-Finance/index\|Corporate Finance]] | Три отчёта, юнит-экономика, WACC, NPV, IRR, M&A, бюджетирование |
| 03 | [[03-Management-Accounting/index\|Management Accounting]] | ABC-costing, Cost-to-Serve, TCO-решения, GAAP / IFRS / РСБУ |

### Уровень B — Функциональная глубина

| # | Модуль | Что внутри |
|---|--------|-----------|
| 04 | [[04-Supply-Chain/index\|Supply Chain Management]] | SCOR, Network Design, Demand Planning (ML), Inventory (EOQ, Newsvendor, MEIO), Logistics, Risk |
| 05 | [[05-Procurement/index\|Strategic Sourcing & Procurement]] | Kraljic, Carter 10C, TCO, SRM, контракты, CIPS / ISM |
| 06 | [[06-Foreign-Trade/index\|Foreign Trade (ВЭД)]] | Incoterms 2020, FX-хеджирование, L/C, China sourcing, геополитика |
| 07 | [[07-Category-Management/index\|Category Management]] | ECR, 8-step Nielsen, Range Review, Pricing, EDLP vs Hi-Lo, Planogramming |

### Уровень C — Продукт и рынок

| # | Модуль | Что внутри |
|---|--------|-----------|
| 08 | [[08-Product-RD/index\|Product Management & R&D]] | Stage-Gate, JTBD, Kano, Design Thinking, GTM, Lifecycle |
| 09 | [[09-Ecom-Marketplaces/index\|E-commerce & Marketplaces]] | Wildberries / Ozon / Amazon-алгоритмы, юнит-экономика, D2C, CRO |
| 10 | [[10-Marketing/index\|Marketing & Consumer Behavior]] | STP, 4P, Brand Equity, MMM, Cialdini, Kahneman |

### Уровень D — Данные, системы, operations

| # | Модуль | Что внутри |
|---|--------|-----------|
| 11 | [[11-Analytics-BI/index\|Analytics & BI]] | SQL, Power BI deep, statistics, A/B-тесты, ML для операций |
| 12 | [[12-ERP-Digital/index\|ERP & Digitalization]] | SAP, Oracle, 1С углубление, PIM, MDM, RPA, Process Mining |
| 13 | [[13-Operations-Excellence/index\|Operations Excellence]] | ISO 9001 / 14001 / 22716, TPM, OEE, Hayes-Wheelwright + **встроен [[13-Operations-Excellence/Lean/index|Lean]]** |
| 14 | [[14-Planning/index\|Planning Methodologies]] | **встроены [[14-Planning/SOP/index|S&OP]] и [[14-Planning/Other-methodologies/index|Other methodologies]]** |
| 15 | [[15-Project-Management/index\|Project & Portfolio Management]] | PMBOK 7, Agile / Scrum для не-IT, PMO |

### Уровень E — Лидерский слой

| # | Модуль | Что внутри |
|---|--------|-----------|
| 16 | [[16-Leadership/index\|Leadership & Executive Presence]] | Situational, Servant, Lencioni, Psychological Safety, SBI feedback |
| 17 | [[17-Goal-Setting/index\|Goal Setting]] | **встроена [[17-Goal-Setting/OKR-KPI/index|OKR / KPI / BSC]]** + расширение |
| 18 | [[18-HR-Management/index\|HR Management]] | Hiring, Performance Mgmt, 9-Box, Total Rewards |
| 19 | [[19-Org-Design-Change/index\|Org Design & Change Management]] | McKinsey 7S, Kotter, ADKAR, Watkins First 90 Days |
| 20 | [[20-Negotiation/index\|Negotiation (Advanced)]] | 7 школ (Harvard / Ury / Lax-Sebenius / Voss / Camp / Рызов / Meyer) + сравнение + playbook силы + протокол подготовки |

### Уровень F — Контроль и горизонт

| # | Модуль | Что внутри |
|---|--------|-----------|
| 21 | [[21-Legal/index\|Legal & Compliance]] | Договорное, корпоративное, трудовое право, антимоно, ПДн, антикоррупция |
| 22 | [[22-Risk-BC/index\|Risk Management & Business Continuity]] | COSO ERM, ISO 31000, BCM, NIST Cybersecurity |
| 23 | [[23-ESG/index\|ESG & Sustainability]] | GRI, SASB, TCFD, CSRD, Circular Economy |
| 24 | [[24-Cross-Cultural/index\|Cross-Cultural Management]] | Hofstede, Erin Meyer, China-specific |
| 25 | [[25-Academic-Track/index\|Academic Track (MBA / PhD)]] | Top MBAs, EMBA, кандидатская по логистике, executive education |
| 26 | [[26-Soft-Skills/index\|Soft Skills & Character]] | EI, Systems Thinking, когнитивные искажения, привычки |

### Уровень G — Индустрия-специфичное

| # | Модуль | Что внутри |
|---|--------|-----------|
| 27 | [[27-Industry-Specific/index\|Industry-Specific Deep Dives]] | Beauty / Cosmetics, FMCG, E-com (Wildberries / Ozon), Automotive aftermarket |

### Кросс-модульные

| Раздел | Что внутри |
|--------|-----------|
| [[Compare/index\|Compare]] | Матрица выбора методов под ситуацию |
| [[Glossary/index\|Glossary]] | Словарь терминов, метрик, аббревиатур, концепций |

## Размещение ранних папок в иерархии (реструктуризация 2026-06-01)

Ранние папки физически перемещены вложенными подпапками внутрь профильных модулей (раньше торчали на верхнем уровне). Все wikilinks обновлены.

| Папка (новое расположение) | Входит в | Комментарий |
|-----------------|-------------------|-------------|
| `14-Planning/SOP/` | [[14-Planning/index\|14 Planning Methodologies]] | S&OP — основа модуля |
| `14-Planning/Other-methodologies/` | [[14-Planning/index\|14 Planning Methodologies]] | IBP, MRP II, DDMRP, ToC, Hoshin Kanri, ZBB, EOS |
| `13-Operations-Excellence/Lean/` | [[13-Operations-Excellence/index\|13 Operations Excellence]] | Lean Production — основа модуля |
| `17-Goal-Setting/OKR-KPI/` | [[17-Goal-Setting/index\|17 Goal Setting]] | OKR, KPI, BSC — основа модуля |
| `Compare/` | Кросс-модульное | Матрица выбора методов |
| `Glossary/` | Кросс-модульное | Словарь терминов |
| `Forecasting/` | Кросс-модульное | Прогнозирование спроса (методы, M-competitions, системы) |
| `System/` | Отдельная вертикаль | Личный AI-стек, не публикуется |

## Самые важные заметки (если читать только 5) — наследие

1. [[17-Goal-Setting/OKR-KPI/06-OKR-vs-KPI-the-difference|OKR vs KPI — the difference]]
2. [[Compare/02-Decision-matrix|Decision matrix]]
3. [[Compare/04-Cases-by-situation|Cases by situation]]
4. [[14-Planning/SOP/04-Cases|S&OP — реальные кейсы]]
5. [[13-Operations-Excellence/Lean/07-For-the-manager|Lean для руководителя]]

## Карта методологий

![[methodology-landscape.png]]
