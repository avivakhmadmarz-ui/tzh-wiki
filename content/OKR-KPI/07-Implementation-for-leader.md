...
- Собери всё, что сейчас называется «целями» или «KPI» в команде.
- Положи в одну таблицу: что именно, кто owner, как часто смотрим, влияет ли на бонус.
- Раздели по столбцам: это KPI (health), это OKR-кандидат (change), это task list (delete).
...
#### Шаг 2. Сформулируй Annual направление
...
#### Шаг 3. Определи KPI-дашборд

- 10-15 KPI в 4 перспективах (Financial / Customer / Internal Process / Learning & Growth).
- Каждый KPI: name, definition (формула), owner, source (откуда берём), target, yellow/red threshold, frequency.
- Положи в одно место — Sheets, Notion, Tableau, Looker — не важно. Главное: **в одном месте, видно всем**.

### Месяц 2 — pilot first quarter OKR

**Не запускай сразу на всю компанию.** Ошибка №1.

#### Шаг 4. Pilot на одной команде

- Выбери одну команду. Лучше — твою непосредственную (управленческую).
- Поставь 3 OKR на этот квартал (а не 5 — нужен фокус для пилота).
- Введи cadence: Monday-commit, Friday-wins.
- Сделай OKR public — все видят, что вы работаете.

#### Шаг 5. Communicate, communicate, communicate

- На all-hands: «Мы пилотируем OKR. Это не оценка performance. Это про фокус.»
- Объясни committed vs aspirational. Объясни 0.6-0.7 sweet spot.
- Объясни, что **бонусы не привязаны** (если не привязаны — повтори трижды).

### Месяц 3 — first retrospective

#### Шаг 6. Конец квартала

- Каждый KR оцени 0.0-1.0 публично.
- **Что сработало? Что нет? Почему?**
- Что переносим в следующий квартал, что отбрасываем.
- Корректируй формат: возможно, в твоей культуре Friday-celebration не зашёл, а Tuesday-standup лучше работает.

#### Шаг 7. Расширение

- Q+1 — добавь 1-2 команды.
- Q+2 — ещё 2-3.
- Q+3 — компания целиком.

**Не торопись.** Adobe внедрял Check-in **2 года**. Spotify шлифовал свой подход **5+ лет**. У тебя **нет** дедлайна на «всё внедрено за квартал».
...
**Лечение:** Hard rule — 3 OKR maximum per team per quarter. 5 — потолок для company-level. Каждое добавление — на встрече leadership team аргументируешь.

### 3. OKR как task list

KR пишутся как deliverables: «Запустить фичу X», «Нанять 5 людей», «Провести 3 тренинга».

**Лечение:** на каждом KR спрашивай — *«если мы это сделаем, изменится ли исход для бизнеса?»*. Если ответ «не обязательно» — это не KR. Перепиши на outcome: «Достичь X retention», «Команда укомплектована для скейла Y».
...
**Не выбирай tool первым.** Сначала год работай в Sheets/Notion. Когда боль из «не помещается в таблицу» становится ощутимой — тогда выбирай. Большинство команд **никогда** не дорастают до specialized software.
...
- [ ] У меня есть 3-5 annual OKR на уровень компании, сформулированных лично.
- [ ] У меня есть KPI-дашборд с 10-15 health-метриками в 4 перспективах.
- [ ] Каждый KPI имеет owner, target, yellow/red threshold, frequency.
- [ ] Я объяснил команде: OKR ≠ performance review, не привязан к бонусу.
...
- [ ] Я лично написал свои публичные OKR и показал их первым.
- [ ] У нас есть план retrospective в конце квартала.
...
- [ ] Каждый говорит: что сделаю на этой неделе для движения KR.
...
## Связь с твоей ситуацией (beauty-импорт, ритейл, операционка)
...
**Не пытайся заменить весь management OKR-ами.** Ритейл живёт на KPI. OKR — для change.
